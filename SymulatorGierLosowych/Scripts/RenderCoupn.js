function RenderCoupon(quantity, numbersDiv, resDiv) {
    ...
    if (count < 100 & count >= 0) {
        ...
        for (var j = 1; j <= count; j++) {
            var counter = 2;
            n += '<div style="float:left; width:350px; padding:5px;">';
            ...
            for (var i = 1; i <= 49; i++) {
                ...
                if (counter == 11) {
                    n += '<div class="number" type="button" id="' + j + "|" + i + '" style="float:left; width:30px; text-align:right; cursor: pointer;">' + i + '</div>';
                    //n += '<div style="float:left; width:30px; background-color: #dddddd"></div>';
                    n += '<br />';
                    counter = 1;
                }
                counter++;
            ...
            n += '</div>';
            //n += '<br /><br />';
            //n += '<div style="float:left; width:30px; background-color: #ffffff;"></div>';
        }
        n += '<br />';
        n += '</div>';
    }
